...
// PkScript struct.
func isSupportedScriptType(class domain.ScriptClass) bool {
	switch class {
	case domain.PubKeyHashTy, domain.WitnessV0PubKeyHashTy, domain.ScriptHashTy,
		domain.WitnessV0ScriptHashTy:
		return true
	default:
		return false
	}
...
func (s PkScript) Class() domain.ScriptClass {
	return s.class
}
...
func computeNonWitnessPkScript(sigScript []byte) (PkScript, error) {
	...
	case len(sigScript) >= minPubKeyHashSigScriptLen &&
		...
		if btcec.IsCompressedPubKey(pubKey) {
			...
			pkScript := PkScript{class: domain.PubKeyHashTy}
	...
	default:
		...
		pkScript := PkScript{class: domain.ScriptHashTy}
		copy(pkScript.script[:], script)
		return pkScript, nil
	}
}
...
func computeWitnessPkScript(witness wire.TxWitness) (PkScript, error) {
	...
	case len(witness) == 2 && len(lastWitnessItem) == compressedPubKeyLen:
		...
		pkScript.class = domain.WitnessV0PubKeyHashTy
		copy(pkScript.script[:], script)
	...
	default:
		...
		pkScript.class = domain.WitnessV0ScriptHashTy
		copy(pkScript.script[:], script)
	}
