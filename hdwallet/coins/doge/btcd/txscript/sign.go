...
import (
	...
	"git.mazdax.tech/blockchain/hdwallet/coins/doge/domain"
...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	amt util.Amount, subScript []byte, hashType SigHashType,
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt util.Amount,
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, addresses []*domain.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		for _, addr := range pkScript.Addresses {
			...
			// bad input.
			pkaddr := addr.DogeAddr.(*btcutil.AddressPubKey)
...
func (kc KeyClosure) GetKey(address *domain.Address) (*btcec.PrivateKey,
	bool, error) {
	return kc(address)
}
