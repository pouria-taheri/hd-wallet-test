...
func calcWitnessSignatureHash(subScript []parsedOpcode, sigHashes *TxSigHashes,
	hashType SigHashType, tx *wire.MsgTx, idx int, amt util.Amount) ([]byte, error) {
	...
	// signed.
	var bAmount [8]byte
	binary.LittleEndian.PutUint64(bAmount[:], amt.ToUint64())
...
func CalcWitnessSigHash(script []byte, sigHashes *TxSigHashes, hType SigHashType,
	tx *wire.MsgTx, idx int, amt util.Amount) ([]byte, error) {
...
func calcSignatureHash(script []parsedOpcode, hashType SigHashType, tx *wire.MsgTx, idx int) []byte {
	...
	case SigHashSingle:
		...
		for i := 0; i < idx; i++ {
			txCopy.TxOut[i].Value = util.AmountFromInt(-1)
