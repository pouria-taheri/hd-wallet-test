...
func typeOfScript(pops []parsedOpcode) domain.ScriptClass {
	if isPubkey(pops) {
		...
		return domain.WitnessV0PubKeyHashTy
	} else if isScriptHash(pops) {
		return domain.ScriptHashTy
	} else if isWitnessScriptHash(pops) {
		return domain.WitnessV0ScriptHashTy
...
func expectedInputs(pops []parsedOpcode, class domain.ScriptClass) int {
	...
	case domain.WitnessV0PubKeyHashTy:
		return 2
	...
	case domain.WitnessV0ScriptHashTy:
		// Not including script.  That is handled by the caller.
		return 1
...
func CalcScriptInfo(sigScript, pkScript []byte, witness wire.TxWitness,
	...
	case si.PkScriptClass == domain.ScriptHashTy && bip16 && !segwit:
	...
	case si.PkScriptClass == domain.WitnessV0PubKeyHashTy && segwit:
	...
	case si.PkScriptClass == domain.ScriptHashTy &&
	...
	case si.PkScriptClass == domain.WitnessV0ScriptHashTy && segwit:
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (domain.ScriptClass, []btcutil.Address, int, error) {
	...
	if err != nil {
		return domain.NonStandardTy, nil, 0, err
	}
