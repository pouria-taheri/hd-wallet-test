...
import (
	...
	"git.mazdax.tech/blockchain/hdwallet/coins/ltc/account/domain"
...
func New(logger logger.Logger, registry configcore.Registry,
	...
	case "testnet4":
		uc.ChainParams = &chaincfg.TestNet4Params
	case "simnet":
...
func (uc *useCase) GetKey(addr *bd.Address) (*btcec.PrivateKey, bool, error) {
...
func (uc *useCase) GetScript(addr *bd.Address) ([]byte, error) {
	ma, err := uc.GetManagedAddress(ad.Request{
		DerivationPath: ad.DerivationPath{
			Account: addr.Account,
			Branch:  addr.Branch,
