...
func calcWitnessSignatureHash(subScript []parsedOpcode, sigHashes *TxSigHashes,
	hashType domain.SigHashType, tx *wire.MsgTx, idx int, amt int64) ([]byte, error) {
	...
	if hashType&domain.SigHashAnyOneCanPay == 0 {
	...
	// hashSequence.
	if hashType&domain.SigHashAnyOneCanPay == 0 &&
		hashType&domain.SigHashMask != domain.SigHashSingle &&
		hashType&domain.SigHashMask != domain.SigHashNone {
	...
	// pre-image.
	if hashType&domain.SigHashSingle != domain.SigHashSingle &&
		hashType&domain.SigHashNone != domain.SigHashNone {
		sigHash.Write(sigHashes.HashOutputs[:])
	} else if hashType&domain.SigHashMask == domain.SigHashSingle &&
		idx < len(tx.TxOut) {
		var b bytes.Buffer
...
func CalcWitnessSigHash(script []byte, sigHashes *TxSigHashes,
	hType domain.SigHashType, tx *wire.MsgTx, idx int,
	amt int64) ([]byte, error) {
...
// verification.
func CalcSignatureHash(script []byte, hashType domain.SigHashType,
...
// engine instance, calculate the signature hash to be used for signing and
// verification.
func calcSignatureHash(script []parsedOpcode, hashType domain.SigHashType,
	tx *wire.MsgTx, idx int) []byte {
	...
	if hashType&domain.SigHashMask == domain.SigHashSingle && idx >= len(tx.TxOut) {
	...
	switch hashType & domain.SigHashMask {
	case domain.SigHashNone:
	...
	default:
		...
		fallthrough
	case domain.SigHashOld:
		fallthrough
	case domain.SigHashAll:
	...
	for i, pop := range pops {
		...
		case OP_CHECKMULTISIGVERIFY:
			...
			if precise && i > 0 &&
				...
				nSigs += domain.MaxPubKeysPerMultiSig
			}
		default:
			// Not a sigop.
		}
...
func getWitnessSigOps(pkScript []byte, witness wire.TxWitness) int {
	...
	case 0:
		switch {
		case len(witnessProgram) == domain.PayToWitnessPubKeyHashDataSize:
			return 1
		case len(witnessProgram) == domain.PayToWitnessScriptHashDataSize &&
			len(witness) > 0:
