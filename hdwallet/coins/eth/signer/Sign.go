...
func NewSigner(logger logger.Logger, conf configcore.Registry, wallet domain.ETHWallet) baseDomain.SignerModel {
	sc := new(signerConf)
	_ = conf.Unmarshal(sc)
	s := Signer{
		wallet:    wallet,
		eipSigner: types.NewLondonSigner(big.NewInt(sc.NetworkId)),
		logger:    logger,
	}
	return s
}
