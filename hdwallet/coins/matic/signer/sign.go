...
func NewSigner(logger logger.Logger, conf configcore.Registry, wallet domain.MaticWallet) Signer {
	sc := new(signerConf)
	_ = conf.Unmarshal(sc)
	s := Signer{
		Wallet:    wallet,
...
func pathFromUserID(uid uint64, index uint32) (accounts.DerivationPath, error) {
	...
	path := fmt.Sprintf("m/44/966/%s/0/%d", str, index)
