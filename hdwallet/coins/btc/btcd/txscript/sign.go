...
func RawTxInWitnessSignature(sigHashes *TxSigHashes,
	amt int64, tx *wire.MsgTx, idx int,
	subScript []byte, hashType domain.SigHashType,
...
func WitnessSignature(sigHashes *TxSigHashes, amt int64, tx *wire.MsgTx, idx int,
	subscript []byte, hashType domain.SigHashType, privKey *btcec.PrivateKey,
	compress bool) (wire.TxWitness, error) {

	sig, err := RawTxInWitnessSignature(sigHashes, amt, tx, idx, subscript,
		hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType domain.SigHashType, key *btcec.PrivateKey) ([]byte, error) {
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte,
	hashType domain.SigHashType, privKey *btcec.PrivateKey,
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte,
	hashType domain.SigHashType, privKey *btcec.PrivateKey) ([]byte, error) {
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte,
	hashType domain.SigHashType, addresses []*domain.Address,
...
func sign(tx *wire.MsgTx, idx int, extractedScript domain.ExtractedPKScript,
	subScript []byte, hashType domain.SigHashType, kdb KeyDB,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		tSig := sig[:len(sig)-1]
		hashType := domain.SigHashType(sig[len(sig)-1])
		...
		for _, addr := range pkScript.Addresses {
			...
			// bad input.
			pkaddr := addr.BtcAddr.(*btcutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(addr *domain.Address) (*btcec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(addr *domain.Address) (*btcec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(addr *domain.Address) (*btcec.PrivateKey, bool, error) {
	return kc(addr)
}
...
type ScriptDB interface {
	GetScript(addr *domain.Address) ([]byte, error)
}
...
func SignTxOutput(chainParams *chaincfg.Params,
	...
	hashType domain.SigHashType, kdb KeyDB, sdb ScriptDB,
