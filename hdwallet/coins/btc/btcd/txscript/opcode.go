...
func opcodeNop(op *parsedOpcode, vm *Engine) error {
	...
	case OP_NOP1, OP_NOP4, OP_NOP5,
		...
		if vm.hasFlag(domain.ScriptDiscourageUpgradableNops) {
...
func popIfBool(vm *Engine) (bool, error) {
	...
	if !vm.isWitnessVersionActive(0) ||
		!vm.hasFlag(domain.ScriptVerifyMinimalIf) {
...
func opcodeCheckLockTimeVerify(op *parsedOpcode, vm *Engine) error {
	...
	if !vm.hasFlag(domain.ScriptVerifyCheckLockTimeVerify) {
		if vm.hasFlag(domain.ScriptDiscourageUpgradableNops) {
...
func opcodeCheckSequenceVerify(op *parsedOpcode, vm *Engine) error {
	...
	if !vm.hasFlag(domain.ScriptVerifyCheckSequenceVerify) {
		if vm.hasFlag(domain.ScriptDiscourageUpgradableNops) {
			return scriptError(ErrDiscourageUpgradableNOPs,
...
func opcodeCheckSig(op *parsedOpcode, vm *Engine) error {
	...
	hashType := domain.SigHashType(fullSigBytes[len(fullSigBytes)-1])
	...
	if vm.hasFlag(domain.ScriptVerifyStrictEncoding) ||
		vm.hasFlag(domain.ScriptVerifyDERSignatures) {
	...
	if !valid && vm.hasFlag(domain.ScriptVerifyNullFail) && len(sigBytes) > 0 {
...
func opcodeCheckMultiSig(op *parsedOpcode, vm *Engine) error {
	...
	if numPubKeys > domain.MaxPubKeysPerMultiSig {
		str := fmt.Sprintf("too many pubkeys: %d > %d",
			numPubKeys, domain.MaxPubKeysPerMultiSig)
		return scriptError(ErrInvalidPubKeyCount, str)
	}
	...
	if vm.hasFlag(domain.ScriptStrictMultiSig) && len(dummy) != 0 {
	...
	for numSignatures > 0 {
		...
		hashType := domain.SigHashType(rawSig[len(rawSig)-1])
	...
	if !success && vm.hasFlag(domain.ScriptVerifyNullFail) {
