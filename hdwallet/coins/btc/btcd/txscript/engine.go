...
func (vm *Engine) hasFlag(flag domain.ScriptFlags) bool {
...
func (vm *Engine) executeOpcode(pop *parsedOpcode) error {
	...
	if pop.opcode.value > OP_16 {
		vm.numOps++
		if vm.numOps > domain.MaxOpsPerScript {
			str := fmt.Sprintf("exceeded max operation limit of %d",
				domain.MaxOpsPerScript)
			return scriptError(ErrTooManyOperations, str)
		}

	} else if len(pop.data) > domain.MaxScriptElementSize {
		str := fmt.Sprintf("element size %d exceeds max allowed size %d",
			len(pop.data), domain.MaxScriptElementSize)
...
func (vm *Engine) verifyWitnessProgram(witness [][]byte) error {
	if vm.isWitnessVersionActive(0) {
		...
		case domain.PayToWitnessPubKeyHashDataSize: // P2WKH
		...
		case domain.PayToWitnessScriptHashDataSize: // P2WSH
			...
			if len(witnessScript) > domain.MaxScriptSize {
				str := fmt.Sprintf("witnessScript size %d "+
					"is larger than max allowed size %d",
					len(witnessScript), domain.MaxScriptSize)
		...
		default:
			errStr := fmt.Sprintf("length of witness program "+
				...
				domain.PayToWitnessPubKeyHashDataSize,
				domain.PayToWitnessScriptHashDataSize,
				len(vm.witnessProgram))
			return scriptError(ErrWitnessProgramWrongLength, errStr)
		}
	} else if vm.hasFlag(domain.ScriptVerifyDiscourageUpgradeableWitnessProgram) {
	...
	if vm.isWitnessVersionActive(0) {
		...
		for _, witElement := range vm.GetStack() {
			if len(witElement) > domain.MaxScriptElementSize {
				str := fmt.Sprintf("element size %d exceeds "+
					"max allowed size %d", len(witElement),
					domain.MaxScriptElementSize)
				return scriptError(ErrElementTooBig, str)
			}
		}
	}
...
func (vm *Engine) CheckErrorCondition(finalScript bool) error {
	...
	if finalScript && vm.hasFlag(domain.ScriptVerifyCleanStack) &&
		vm.dstack.Depth() != 1 {
...
func (vm *Engine) Step() (done bool, err error) {
	...
	if combinedStackSize > domain.MaxStackSize {
		str := fmt.Sprintf("combined stack size %d > max allowed %d",
			combinedStackSize, domain.MaxStackSize)
...
func (vm *Engine) checkHashTypeEncoding(hashType domain.SigHashType) error {
	if !vm.hasFlag(domain.ScriptVerifyStrictEncoding) {
		return nil
	}

	sigHashType := hashType & ^domain.SigHashAnyOneCanPay
	if sigHashType < domain.SigHashAll || sigHashType > domain.SigHashSingle {
...
func (vm *Engine) checkPubKeyEncoding(pubKey []byte) error {
	if vm.hasFlag(domain.ScriptVerifyWitnessPubKeyType) &&
...
func (vm *Engine) checkSignatureEncoding(sig []byte) error {
	if !vm.hasFlag(domain.ScriptVerifyDERSignatures) &&
		!vm.hasFlag(domain.ScriptVerifyLowS) &&
		!vm.hasFlag(domain.ScriptVerifyStrictEncoding) {
	...
	if vm.hasFlag(domain.ScriptVerifyLowS) {
...
func NewEngine(scriptPubKey []byte, tx *wire.MsgTx, txIdx int,
	flags domain.ScriptFlags, sigCache *SigCache, hashCache *TxSigHashes,
	inputAmount int64) (*Engine, error) {
	...
	if vm.hasFlag(domain.ScriptVerifyCleanStack) &&
		(!vm.hasFlag(domain.ScriptBip16) &&
			!vm.hasFlag(domain.ScriptVerifyWitness)) {
	...
	if vm.hasFlag(domain.ScriptVerifySigPushOnly) &&
		!IsPushOnlyScript(scriptSig) {
	...
	for i, scr := range scripts {
		if len(scr) > domain.MaxScriptSize {
			str := fmt.Sprintf("script size %d is larger than max "+
				"allowed size %d", len(scr), domain.MaxScriptSize)
	...
	if vm.hasFlag(domain.ScriptBip16) && isScriptHash(vm.scripts[1]) {
	...
	if vm.hasFlag(domain.ScriptVerifyMinimalData) {
	...
	if vm.hasFlag(domain.ScriptVerifyWitness) {
		// If witness evaluation is enabled, then P2SH MUST also be
		// active.
		if !vm.hasFlag(domain.ScriptBip16) {
