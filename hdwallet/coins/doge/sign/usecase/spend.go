...
import (
	"git.mazdax.tech/blockchain/hdwallet/coins/doge/btcd/txauthor"
	"git.mazdax.tech/blockchain/hdwallet/coins/doge/btcd/txscript"
	"git.mazdax.tech/blockchain/hdwallet/coins/doge/btcd/util"
	"git.mazdax.tech/blockchain/hdwallet/coins/doge/btcd/wire"
...
func (uc *signer) SpendWitnessKeyHash(
	inputAddress *dd.Address, chainParams *chaincfg.Params,
	hashCache *txscript.TxSigHashes, amt util.Amount, tx *wire.MsgTx, idx int,
	...
	witnessScript, err := txscript.WitnessSignature(tx, hashCache, idx, amt,
		witnessProgram, txscript.SigHashAll,
		privKey, true)
	if err != nil {
		uc.logger.With(logger.Field{
			"section": "SpendWitnessKeyHash",
...
func (uc *signer) SpendNestedWitnessPubKeyHash(inputAddress *dd.Address,
	chainParams *chaincfg.Params, hashCache *txscript.TxSigHashes, amt util.Amount,
	...
	witnessScript, err := txscript.WitnessSignature(tx, hashCache, idx, amt,
		sigScript, txscript.SigHashAll, privKey, compressed)
	if err != nil {
		uc.logger.With(logger.Field{
			"section": "SpendNestedWitnessPubKeyHash",
			"error":   err,
